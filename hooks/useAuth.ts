...
export const useAuth = () => {
  ...
  // Giriş yapma
  const signIn = async ({ email, password }: SignInData): Promise<{ error?: string }> => {
    try {
  ...
  // Kayıt olma
  const signUp = async ({ email, password, fullName }: SignUpData): Promise<{ error?: string }> => {
    try {
      setAuthState(prev => ({ ...prev, loading: true, error: null }));
  ...
  // Profil güncelleme
  const updateProfile = async (updates: { full_name?: string; avatar_url?: string }): Promise<{ error?: string }> => {
    try {
      if (!authState.user) {
